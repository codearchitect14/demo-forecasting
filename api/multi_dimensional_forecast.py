...
class WeatherRecommendation(BaseModel):
    ...
    recommendation_type: (
        str  # 'increase_stock', 'decrease_stock', 'normal', 'bundle_opportunity'
    )
...
async def demand_forecast(request_body: dict, request: Request):
    ...
    try:
        ...
        async with manager.get_connection() as conn:
            ...
            demand_forecasts = await generate_demand_forecasts(
                conn, city_ids, store_ids, product_ids, forecast_days
            )

            # Calculate stockout risk analysis
            stockout_risk_analysis = await calculate_stockout_risk_analysis(
                conn, city_ids, store_ids, product_ids, demand_forecasts
            )

            # Generate demand insights
            demand_insights = await generate_demand_insights(
                conn, inventory_status, demand_forecasts, stockout_risk_analysis
            )
        ...
        for store_id in store_ids:
            for product_id in product_ids:
                try:
                    ...
                    current_stock = await estimate_current_stock_level(
                        conn, city_id, store_id, product_id
                    )

                    # Get stockout frequency from historical data
                    stockout_freq = await get_stockout_frequency(
                        conn, city_id, store_id, product_id
                    )

                    # Get average daily demand
                    avg_daily_demand = await get_average_daily_demand(
                        conn, city_id, store_id, product_id
                    )

                    # Get last stockout date
                    last_stockout = await get_last_stockout_date(
                        conn, city_id, store_id, product_id
                    )

                    # Calculate stockout risk score
                    stockout_risk_score = calculate_stockout_risk_score(
                        current_stock, avg_daily_demand, stockout_freq
                    )
                    ...
                    recommended_reorder = calculate_recommended_reorder_quantity(
                        avg_daily_demand, stockout_freq
                    )
                    ...
                    logger.error(
                        f"Error getting inventory status for {city_id}-{store_id}-{product_id}: {e}"
                    )
                    continue
...
async def estimate_current_stock_level(
    conn: asyncpg.Connection, city_id: str, store_id: str, product_id: int
) -> int:
    """
    ...
    try:
        ...
        if row and row["total_days"] and row["total_days"] > 0:
            ...
            )  # 7-21 days based on availability
            ...
            if recent_stockout_hours > 12:  # Recently had major stockout
                base_stock = max(
                    int(daily_demand * 2), base_stock - int(daily_demand * 3)
                )
            elif recent_stockout_hours > 6:  # Recently had minor stockout
                base_stock = max(
                    int(daily_demand * 3), base_stock - int(daily_demand * 2)
                )

            # Add some randomness based on sales volatility and product characteristics
            volatility_factor = (
                1.0 + (avg_stockout_hours / 16.0) * 0.3
            )  # Max 30% variation

            # Add product-specific variation based on product_id
            product_variation = (
                (product_id % 100) / 100.0
            ) * 0.4 + 0.8  # 0.8 to 1.2 range

            # Add location-based variation
            location_hash = hash(f"{city_id}_{store_id}") % 100
            location_variation = (
                location_hash / 100.0
            ) * 0.3 + 0.85  # 0.85 to 1.15 range

            base_stock = int(
                base_stock * volatility_factor * product_variation * location_variation
            )
...
async def get_stockout_frequency(
    ...
    try:
        ...
        if row and row["total_days"] and row["total_days"] > 0:
            ...
            product_stockout_factor = (
                (product_id % 20) / 20.0
            ) * 0.3 + 0.85  # 0.85 to 1.15 range

            # Add location-based variation to stockout frequency
            location_hash = hash(f"{city_id}_{store_id}") % 100
            location_stockout_factor = (
                location_hash / 100.0
            ) * 0.2 + 0.9  # 0.9 to 1.1 range
            ...
            base_default = 0.05
            product_default_factor = (
                (product_id % 15) / 15.0
            ) * 0.4 + 0.8  # 0.8 to 1.2 range

            # Add location-based variation to default
            location_hash = hash(f"{city_id}_{store_id}") % 100
            location_default_factor = (
                location_hash / 100.0
            ) * 0.2 + 0.9  # 0.9 to 1.1 range

            final_default = (
                base_default * product_default_factor * location_default_factor
            )
...
async def get_average_daily_demand(
    conn: asyncpg.Connection, city_id: str, store_id: str, product_id: int
) -> float:
    """
    ...
    try:
        ...
        if row and row["total_days"] and row["total_days"] > 0:
            ...
            if days_no_stockout > 0 and avg_sales_no_stockout > 0:
                ...
                base_demand = (
                    avg_sales_no_stockout / 5.0
                )  # Assume $5 average unit price
                ...
                if days_with_stockout > 0 and avg_sales_with_stockout > 0:
                    ...
                    lost_sales_factor = (
                        1.0 + (avg_stockout_hours / 16.0) * 0.5
                    )  # Up to 50% lost sales
            ...
            product_demand_factor = (
                (product_id % 50) / 50.0
            ) * 0.6 + 0.7  # 0.7 to 1.3 range

            # Add location-based variation to demand
            location_hash = hash(f"{city_id}_{store_id}") % 100
            location_demand_factor = (
                location_hash / 100.0
            ) * 0.4 + 0.8  # 0.8 to 1.2 range

            final_demand = (
                adjusted_demand * product_demand_factor * location_demand_factor
            )
            ...
            base_fallback = 8.0
            product_fallback_factor = (
                (product_id % 30) / 30.0
            ) * 0.8 + 0.6  # 0.6 to 1.4 range

            # Add location-based variation to fallback
            location_hash = hash(f"{city_id}_{store_id}") % 100
            location_fallback_factor = (
                location_hash / 100.0
            ) * 0.4 + 0.8  # 0.8 to 1.2 range

            final_fallback = (
                base_fallback * product_fallback_factor * location_fallback_factor
            )
...
async def get_last_stockout_date(
    conn: asyncpg.Connection, city_id: str, store_id: str, product_id: int
) -> Optional[str]:
    """
...
def calculate_stockout_risk_score(
    current_stock: int, avg_daily_demand: float, stockout_frequency: float
) -> float:
    """
...
def calculate_recommended_reorder_quantity(
    avg_daily_demand: float, stockout_frequency: float
) -> int:
    """
    Calculate recommended reorder quantity
    """
        ...
        for store_id in store_ids:
            for product_id in product_ids:
                try:
                    ...
                    historical_data = await get_historical_demand_data(
                        conn, city_id, store_id, product_id
                    )

                    if not historical_data.empty:
                        # Generate demand forecast
                        forecast = await generate_demand_forecast_single(
                    ...
                    logger.error(
                        f"Error generating demand forecast for {city_id}-{store_id}-{product_id}: {e}"
                    )
                    continue
...
async def get_historical_demand_data(
    conn: asyncpg.Connection, city_id: str, store_id: str, product_id: int
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        df["avg_temperature"] = df["avg_temperature"].fillna(
            df["avg_temperature"].mean()
        )
...
async def generate_demand_forecast_single(
    historical_data: pd.DataFrame, forecast_days: int
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        confidence_intervals = calculate_demand_confidence_intervals(
            predictions, historical_data["estimated_units_sold"]
        )
        ...
        return {
            ...
            "peak_demand_day": future_dates[np.argmax(predictions)].strftime(
                "%Y-%m-%d"
            ),
...
def prepare_demand_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        df["demand_ma7"] = (
            df["estimated_units_sold"].rolling(window=7, min_periods=1).mean()
        )
        df["demand_ma30"] = (
            df["estimated_units_sold"].rolling(window=30, min_periods=1).mean()
        )
...
def prepare_future_demand_features(
    historical_data: pd.DataFrame, future_dates: List[datetime]
) -> pd.DataFrame:
    """
    ...
    try:
        ...
        for date in future_dates:
            ...
            features = {
                ...
                "demand_lag7": (
                    recent_data["estimated_units_sold"].iloc[-7]
                    if len(recent_data) >= 7
                    else recent_data["estimated_units_sold"].mean()
                ),
                "demand_lag30": (
                    recent_data["estimated_units_sold"].iloc[-30]
                ...
                "temp_humidity": recent_data["avg_temperature"].mean()
                * recent_data["avg_humidity"].mean(),
            }
...
def calculate_demand_confidence_intervals(
    predictions: np.ndarray, historical_demand: pd.Series
) -> Dict[str, np.ndarray]:
    """
    ...
    try:
        ...
        return {"upper": predictions * 1.3, "lower": predictions * 0.7}
...
def generate_fallback_demand_forecast(forecast_days: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "peak_demand_day": (
            datetime.now() + timedelta(days=predictions.index(max(predictions)) + 1)
        ).strftime("%Y-%m-%d"),
        "model_accuracy": 0.75,
    }


async def calculate_stockout_risk_analysis(
    conn: asyncpg.Connection,
    city_ids: List[str],
    store_ids: List[str],
    product_ids: List[int],
    ...
    try:
                ...
                for product_id in product_ids:
                    ...
                    if key in demand_forecasts:
                        ...
                        if days_until_stockout <= 30:
                            stockout_date = (
                                datetime.now()
                                + timedelta(days=int(days_until_stockout))
                            ).strftime("%Y-%m-%d")
                            ...
                            risk_analysis["stockout_timeline"][stockout_date].append(
                                {
                                    ...
                                    "product_id": product_id,
                                    "product_name": location_info["product_name"],
                                    "estimated_impact": avg_daily_demand
                                    * 50,  # Estimated lost sales
                                }
                            )
        ...
        risk_analysis["risk_factors"] = await analyze_risk_factors(
            conn, city_ids, store_ids, product_ids
        )

        # Generate mitigation strategies
        risk_analysis["mitigation_strategies"] = generate_mitigation_strategies(
            risk_analysis
        )
...
async def generate_demand_insights(
    conn: asyncpg.Connection,
    inventory_status: List[InventoryStatus],
    demand_forecasts: Dict[str, Any],
    stockout_risk_analysis: Dict[str, Any],
) -> List[DemandForecastInsight]:
    """
    ...
    try:
        ...
        for inventory in inventory_status:
            ...
            estimated_lost_sales = (
                inventory.avg_daily_demand * 7 * inventory.stockout_frequency * 5.0
            )  # Assume $5 per unit
...
def generate_inventory_recommendations(
    ...
    try:
        ...
        critical_items = [
            item for item in inventory_status if item.stockout_risk_score >= 0.8
        ]
        if critical_items:
            recommendations.append(
                {
                    ...
                    "estimated_impact": sum(
                        item.avg_daily_demand * 30 * 5 for item in critical_items
        ...
        high_stockout_items = [
            item for item in inventory_status if item.stockout_frequency > 0.3
        ]
            ...
            recommendations.append(
                {
                    ...
                    "estimated_impact": sum(
                        item.avg_daily_demand * 7 * 5 for item in high_stockout_items
                    ),
                }
            )
        ...
        recommendations.append(
            {
                ...
                "estimated_impact": sum(
                    item.avg_daily_demand * 30 * 0.15 for item in inventory_status
                ),  # 15% improvement
            }
        )
...
async def get_diverse_store_selection(
    conn: asyncpg.Connection, city_ids: List[str], store_ids: List[str]
) -> List[str]:
    """
    ...
    try:
        ...
        return (
            selected_stores if selected_stores else ["24", "61", "8", "74", "59"]
        )  # Fallback to diverse stores
...
async def generate_multi_dimensional_forecast(
    ...
    """
    forecast_results = {"combinations": [], "aggregated_data": {}, "time_series": {}}
        ...
        for store_id in store_ids:
            for product_id in product_ids:
                try:
                    ...
                    if not historical_data.empty:
                        forecast = await generate_single_forecast(
                            historical_data, forecast_days
                        )
                    ...
                    logger.error(
                        f"Error forecasting for {city_id}-{store_id}-{product_id}: {e}"
                    )
                    continue
...
async def generate_single_forecast(
    ...
    try:
        ...
        confidence_intervals = calculate_confidence_intervals(
            predictions, historical_data["sale_amount"]
        )
...
def prepare_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        # Create rolling averages
        df["sale_amount_ma7"] = (
            df["sale_amount"].rolling(window=7, min_periods=1).mean()
        )
        df["sale_amount_ma30"] = (
            df["sale_amount"].rolling(window=30, min_periods=1).mean()
        )
        ...
        feature_columns = [
            ...
            "temp_precipitation_interaction",
        ]
    ...
    try:
        ...
        for date in future_dates:
            ...
            features = {
                ...
                "sale_amount_lag7": (
                    recent_data["sale_amount"].iloc[-7]
                    if len(recent_data) >= 7
                    else recent_data["sale_amount"].mean()
                ),
                "sale_amount_lag30": (
                    recent_data["sale_amount"].iloc[-30]
                    if len(recent_data) >= 30
                    else recent_data["sale_amount"].mean()
                ),
                "sale_amount_ma7": recent_data["sale_amount"].tail(7).mean(),
                "sale_amount_ma30": recent_data["sale_amount"].mean(),
                "temp_humidity_interaction": features["temperature"]
                * features["humidity"],
                "temp_precipitation_interaction": features["temperature"]
...
def calculate_confidence_intervals(
    predictions: np.ndarray, historical_sales: pd.Series
) -> Dict[str, np.ndarray]:
    """
    ...
    try:
        ...
        return {"upper": predictions * 1.2, "lower": predictions * 0.8}
...
async def get_location_info(
    ...
    try:
        ...
        # Get store name
        store_query = (
            f"SELECT store_name FROM store_hierarchy WHERE store_id = '{store_id}'"
        )
        ...
        product_name = (
            product_row["product_name"] if product_row else f"Product {product_id}"
        )
...
async def generate_aggregated_forecasts(
    ...
    """
    aggregated = {"by_city": {}, "by_store": {}, "by_product": {}, "total": {}}
    ...
    for product_id in product_ids:
        product_combinations = [
            c for c in combinations if c["product_id"] == product_id
        ]
        if product_combinations:
            aggregated["by_product"][str(product_id)] = aggregate_forecasts(
                product_combinations
            )
...
async def generate_forecast_insights(
    ...
    try:
        ...
        sorted_combinations = sorted(
            combinations, key=lambda x: x["forecast"]["total_predicted"], reverse=True
        )
        ...
        for combo in sorted_combinations:
            ...
            growth_rate = (
                ((predicted_avg - historical_avg) / historical_avg * 100)
                if historical_avg > 0
                else 0
            )
...
def generate_recommendation(combo: Dict, growth_rate: float) -> str:
    ...
    disclaimer = (
        " (Based on market trends and similar products)" if data_points == 0 else ""
    )
...
async def generate_comparative_analysis(
    ...
    try:
        ...
        insights = generate_comparative_insights(
            city_comparison, store_comparison, product_comparison
        )
...
def generate_comparative_insights(
    city_comparison: Dict, store_comparison: Dict, product_comparison: Dict
) -> List[str]:
    """
    Generate comparative insights
    """
    ...
    try:
        ...
        if city_comparison:
            best_city = max(
                city_comparison.items(), key=lambda x: x[1]["total_predicted"]
            )
            insights.append(
                f"In {best_city[0]}, the highest sales volume of ${best_city[1]['total_predicted']:.2f} is expected across all selected products and stores."
            )

        # Best performing store
        if store_comparison:
            best_store = max(
                store_comparison.items(), key=lambda x: x[1]["total_predicted"]
            )
            insights.append(
                f"{best_store[0]} is forecasted to have the highest performance with ${best_store[1]['total_predicted']:.2f} in total sales."
            )

        # Best performing product
        if product_comparison:
            best_product = max(
                product_comparison.items(), key=lambda x: x[1]["total_predicted"]
            )
            insights.append(
                f"{best_product[0]} shows the strongest sales potential with ${best_product[1]['total_predicted']:.2f} expected across all locations."
            )

        # Performance gaps
        if city_comparison and len(city_comparison) > 1:
            sorted_cities = sorted(
                city_comparison.items(),
            ...
            insights.append(
                f"There's a significant performance gap of ${gap:.2f} between {top_city[0]} and {bottom_city[0]}."
            )
...
def generate_forecast_summary(
    forecast_results: Dict[str, Any], insights: List[ForecastInsight]
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        avg_accuracy = sum(c["forecast"]["model_accuracy"] for c in combinations) / len(
            combinations
        )
...
async def weather_api_status():
    ...
    try:
        ...
        return {"status": "error", "message": f"Weather API test failed: {str(e)}"}
...
async def generate_weather_recommendations(
    conn: asyncpg.Connection,
    city_ids: List[str],
    store_ids: List[str],
    product_ids: List[int],
        ...
        for store_id in store_ids:
            for product_id in product_ids:
                try:
                    ...
                    current_weather = await get_current_weather_conditions(
                        conn, city_id
                    )

                    # Get historical weather impact on this product
                    weather_impact = await analyze_weather_impact_on_product(
                        conn, city_id, store_id, product_id
                    )
                    ...
                    logger.error(
                        f"Error generating weather recommendation for {city_id}-{store_id}-{product_id}: {e}"
                    )
                    continue
...
async def get_current_weather_conditions(
    conn: asyncpg.Connection, city_id: str
) -> WeatherCondition:
    """
    ...
    try:
        ...
        if row and row["temperature"] is not None:
            ...
            logger.warning(
                f"No weather data found for city {city_id}, using overall average"
            )
            fallback_query = """
            SELECT 
                AVG(CAST(avg_temperature AS FLOAT)) as temperature,
...
def categorize_weather(
    temperature: float, precipitation: float, humidity: float
) -> str:
    """
...
async def analyze_weather_impact_on_product(
    conn: asyncpg.Connection, city_id: str, store_id: str, product_id: int
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if rows:
            ...
            if len(sales_values) > 1:
                ...
                weather_impact["weather_sensitivity"] = (
                    statistics.stdev(sales_values) / weather_impact["average_sales"]
                )
...
async def generate_weather_recommendation_for_product(
    ...
    try:
        ...
        current_temp_category = (
            "cold"
            if current_weather.temperature < 18
            else "warm" if current_weather.temperature > 25 else "mild"
        )
        current_precip_category = (
            "rainy"
            if current_weather.precipitation > 5
            else "dry" if current_weather.precipitation < 1 else "normal"
        )
        ...
        for condition in best_conditions:
            if (
                condition["temp_category"] == current_temp_category
                and condition["precip_category"] == current_precip_category
            ):
                recommendation_type = "increase_stock"
                impact_percentage = min(
                    50.0,
                    (condition["avg_sales"] / weather_impact["average_sales"] - 1)
                    * 100,
                )
            ...
            for condition in worst_conditions:
                if (
                    ...
                    confidence_score = min(
                        0.8, 0.5 + weather_impact["weather_sensitivity"]
                    )
                    break

        # Generate product-specific weather insights
        product_weather_insights = generate_product_weather_insights(
            product_id, current_weather
        )
        if product_weather_insights:
            ...
            if product_weather_insights["recommendation_type"] != "normal":
                ...
                suggested_stock_adjustment = product_weather_insights[
...
def generate_product_weather_insights(
    product_id: int, current_weather: WeatherCondition
) -> Dict[str, Any]:
    """
    ...
    product_weather_rules = {
        ...
        "warm_weather_boost": {
            "products": [
                118,
                368,
            ],  # Almonds, Alfredo sauce (outdoor activities, cooking)
        ...
        for store_id in store_ids:
            for product_id in product_ids:
                try:
                    ...
                    discount_impact = await analyze_historical_discount_impact(
                        conn, city_id, store_id, product_id, discount_percentage
                    )

                    # Get weather boost factor
                    weather_boost = await calculate_weather_boost_factor(
                        conn, city_id, product_id
                    )

                    # Get holiday boost factor
                    holiday_boost = await calculate_holiday_boost_factor(
                        conn, city_id, store_id, product_id
                    )
                    ...
                    optimal_timing = determine_optimal_timing(
                        weather_boost, holiday_boost
                    )
                    ...
                    logger.error(
                        f"Error analyzing promotional impact for {city_id}-{store_id}-{product_id}: {e}"
                    )
                    continue
...
async def analyze_historical_discount_impact(
    conn: asyncpg.Connection,
    city_id: str,
    store_id: str,
    product_id: int,
...
async def calculate_weather_boost_factor(
    conn: asyncpg.Connection, city_id: str, product_id: int
) -> float:
    """
...
async def calculate_holiday_boost_factor(
    conn: asyncpg.Connection, city_id: str, store_id: str, product_id: int
) -> float:
    """
...
def generate_promotional_recommendation(
...
async def generate_weather_impact_summary(
    conn: asyncpg.Connection,
    city_ids: List[str],
    store_ids: List[str],
    product_ids: List[int],
    ...
    try:
        ...
        if sensitivity_count > 0:
            summary["overall_weather_sensitivity"] = (
                total_sensitivity / sensitivity_count
            )
        ...
        for city_id, weather_data in summary["city_weather_conditions"].items():
            if weather_data["weather_category"] == "cold":
                summary["weather_recommendations"].append(
                    f"City {city_id}: Cold weather - increase stock of indoor products"
                )
            elif weather_data["weather_category"] == "rainy":
                summary["weather_recommendations"].append(
                    f"City {city_id}: Rainy weather - increase stock of indoor/comfort products"
                )
            elif weather_data["weather_category"] == "warm":
                summary["weather_recommendations"].append(
                    f"City {city_id}: Warm weather - increase stock of outdoor/cooking products"
                )
...
async def generate_holiday_impact_summary(
    conn: asyncpg.Connection,
    city_ids: List[str],
    store_ids: List[str],
    product_ids: List[int],
    ...
    try:
        ...
        if summary["holiday_boost_average"] > 1.1:
            ...
            summary["holiday_recommendations"].append(
                "Current holiday period - increase stock and consider promotions"
            )
        elif summary["holiday_boost_average"] > 1.05:
            summary["current_holiday_status"] = "pre_holiday"
            summary["holiday_recommendations"].append(
                "Pre-holiday period - prepare for increased demand"
            )
        else:
            summary["holiday_recommendations"].append(
                "Normal period - standard inventory management"
            )
...
async def generate_weather_bundle_recommendations(
    conn: asyncpg.Connection,
    city_ids: List[str],
    store_ids: List[str],
    product_ids: List[int],
                    ...
                    for product_id in bundle_info["products"]:
                        if product_id in product_ids:
                            location_info = await get_location_info(
                                conn,
                                city_id,
                                store_ids[0] if store_ids else "0",
                                product_id,
                            )
...
async def generate_seasonal_insights(
    conn: asyncpg.Connection,
    city_ids: List[str],
    store_ids: List[str],
    product_ids: List[int],
