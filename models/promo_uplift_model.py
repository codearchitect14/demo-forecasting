...
class PromoUpliftModel:
    ...
    def prepare_data(self, df, target_col="uplift", drop_cols=None):
        ...
        if target_col not in data.columns:
            ...
            avg_sales = (
                data.groupby(group_cols + ["promo_flag"])["sale_amount"]
                .mean()
                .reset_index()
            )
            avg_sales_pivot = avg_sales.pivot_table(
                index=group_cols, columns="promo_flag", values="sale_amount"
            ).reset_index()

            # Rename columns
            avg_sales_pivot.columns.name = None
            avg_sales_pivot = avg_sales_pivot.rename(
                columns={True: "promo_sales", False: "non_promo_sales"}
            )
            ...
            if (
                "promo_sales" not in avg_sales_pivot.columns
                or "non_promo_sales" not in avg_sales_pivot.columns
            ):
            ...
            avg_sales_pivot["uplift"] = (
                avg_sales_pivot["promo_sales"] - avg_sales_pivot["non_promo_sales"]
            )
            avg_sales_pivot["uplift_pct"] = (
                avg_sales_pivot["uplift"] / avg_sales_pivot["non_promo_sales"]
            ) * 100

            # Merge uplift back to original data
            data = pd.merge(
                data,
                avg_sales_pivot[group_cols + ["uplift", "uplift_pct"]],
                on=group_cols,
            )
            target_col = "uplift"
    ...
    def train(self, df, target_col="uplift", drop_cols=None, test_size=0.2):
        ...
        logger.info(
            f"Model evaluation metrics - MAE: {metrics['mae']:.4f}, "
            f"RMSE: {metrics['rmse']:.4f}, R²: {metrics['r2']:.4f}"
        )

        # Cross-validation
        cv_scores = cross_val_score(
            self.pipeline, X, y, cv=5, scoring="neg_mean_absolute_error"
        )
    ...
    def analyze_promotion_effectiveness(self, df, group_cols=None, target_col="uplift"):
        ...
        group_results = (
            analysis_df.groupby(group_cols)
            .agg({target_col: ["mean", "median", "std", "count"]})
            .reset_index()
        )

        # Flatten multi-level column names
        group_results.columns = [
            "_".join(col).strip("_") for col in group_results.columns.values
        ]

        # Add percentile ranks to see which combinations perform best
        group_results[f"{target_col}_percentile"] = (
            group_results[f"{target_col}_mean"].rank(pct=True) * 100
        )
...
if __name__ == "__main__":
    ...
    df.loc[df["promo_flag"], "sale_amount"] *= (
        1 + df.loc[df["promo_flag"], "discount"] * 3
    )
