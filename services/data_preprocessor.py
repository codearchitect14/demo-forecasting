...
class DataPreprocessor:
    ...
    def add_sales_features(self, df):
        ...
        df["hours_stock_status_list"] = df["hours_stock_status"].apply(
            self.process_sequence_data
        )
        ...
        df["stockout_risk"] = df.apply(
            lambda x: (
                ...
                / len(x["hours_stock_status_list"])
                if x["hours_stock_status_list"]
                else 0
            ),
            axis=1,
        )
    ...
    def handle_outliers(self, df):
        ...
        self.outlier_bounds = {
            "sale_amount": {"lower": lower_bound, "upper": upper_bound}
        }
    ...
    def save_preprocessor_state(self):
        ...
        state = {"outlier_bounds": self.outlier_bounds}
