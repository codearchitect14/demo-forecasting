...
async def generate_forecast(
    ...
    if (
        ...
        and request.city_id is not None
...
async def analyze_promotion_effectiveness(
    df: pd.DataFrame, promo_data: pd.DataFrame, model, request
) -> Dict[str, Any]:
    """
            ...
            for _, promo in promo_data.iterrows():
                ...
                if "store_id" in promo:
                    if isinstance(row["store_id"], pd.Series):
                        store_id_match = (
                            row["store_id"] == promo["store_id"]
                        ).any() or pd.isna(promo["store_id"])
                    else:
                        store_id_match = (
                            row["store_id"] == promo["store_id"]
                        ) or pd.isna(promo["store_id"])
                if "product_id" in promo:
                    if isinstance(row["product_id"], pd.Series):
                        product_id_match = (
                            row["product_id"] == promo["product_id"]
                        ).any() or pd.isna(promo["product_id"])
                    else:
                        product_id_match = (
                            row["product_id"] == promo["product_id"]
                        ) or pd.isna(promo["product_id"])
                if (
                    promo_start <= sale_date <= promo_end
                    and bool(store_id_match)
                    and bool(product_id_match)
                ):
    ...
    if (
        hasattr(request, "promotion_type")
        and request.promotion_type is not None
        and "promotion_type" in df.columns
    ):
        if isinstance(df["promotion_type"], pd.Series):
            df = df[df["promotion_type"] == request.promotion_type]
    if (
        hasattr(request, "discount_min")
        and request.discount_min is not None
    ...
    if (
        hasattr(request, "discount_max")
        and request.discount_max is not None
        and "discount_percentage" in df.columns
    ):
        if isinstance(df["discount_percentage"], pd.Series):
...
async def analyze_stockout(df: pd.DataFrame, request) -> Dict[str, Any]:
    ...
    if not df.empty:
        ...
        if isinstance(sale_dates, pd.Series) and not sale_dates.empty:
            avg_sales_by_dow = (
                df.loc[~df["is_stockout"]]
                .groupby(sale_dates.dt.dayofweek)["sale_amount"]
                .mean()
                .to_dict()
            )
        else:
...
async def analyze_holiday_impact(
    df: pd.DataFrame, holidays_df: pd.DataFrame, request
) -> Dict[str, Any]:
    """
