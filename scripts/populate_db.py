...
async def insert_data(conn, table_name, df, conflict_cols=None):
    ...
    records = df.where(
        pd.notnull(df), None
    ).values.tolist()  # Replace NaN with None for SQL NULL
...
async def populate_freshretailnet_data():
    ...
    try:
        ...
        city_hierarchy_df["city_name"] = "City " + city_hierarchy_df["city_id"].astype(
            str
        )  # Dummy name
        city_hierarchy_df["climate_zone"] = "Temperate"  # Dummy value
        await insert_data(
            conn, "city_hierarchy", city_hierarchy_df, conflict_cols=["city_id"]
        )

        # Store Hierarchy (requires city_id)
        store_hierarchy_df = df_raw[["store_id", "city_id"]].drop_duplicates().copy()
        store_hierarchy_df["store_name"] = "Store " + store_hierarchy_df[
            "store_id"
        ].astype(
            str
        )  # Dummy name
        store_hierarchy_df["store_type"] = "Supermarket"  # Dummy value
        store_hierarchy_df["region"] = "Region " + store_hierarchy_df["city_id"].astype(
            str
        )  # Dummy based on city
        # For latitude/longitude, you'd need external data or generate dummy
        store_hierarchy_df["latitude"] = np.random.uniform(
            25.0, 50.0, len(store_hierarchy_df)
        )
        store_hierarchy_df["longitude"] = np.random.uniform(
            -125.0, -70.0, len(store_hierarchy_df)
        )
        store_hierarchy_df["opening_date"] = datetime(2020, 1, 1).date()  # Dummy date
        await insert_data(
            conn, "store_hierarchy", store_hierarchy_df, conflict_cols=["store_id"]
        )
        ...
        product_hierarchy_df["product_name"] = "Product " + product_hierarchy_df[
            "product_id"
        ].astype(
            str
        )  # Dummy name
        ...
        product_hierarchy_df["promotion_elasticity"] = np.random.uniform(
            0.1, 0.5, len(product_hierarchy_df)
        )  # Dummy
        await insert_data(
            conn,
            "product_hierarchy",
        ...
        sales_data_df["sale_qty"] = (
            (sales_data_df["sale_amount"] / 5.0).round().astype(int)
        )  # Assuming $5 unit price
        sales_data_df["original_price"] = (
            5.0  # Assuming constant original price for simplicity
        )
        sales_data_df["holiday_flag"] = sales_data_df["holiday_flag"].astype(bool)
        sales_data_df["promo_flag"] = (
            sales_data_df["discount"] < 1.0
        )  # True if there's a discount
        sales_data_df["stock_hour6_22_cnt"] = (
            sales_data_df["stock_hour6_22_cnt"].fillna(0).astype(int)
        )
        sales_data_df["stock_hour6_14_cnt"] = (
            sales_data_df["stock_hour6_22_cnt"] / 2
        ).astype(
            int
        )  # Dummy split
        sales_data_df["stock_hour14_22_cnt"] = (
            sales_data_df["stock_hour6_22_cnt"] / 2
        ).astype(
            int
        )  # Dummy split
        ...
        sales_data_df["demand_volatility"] = np.random.uniform(
            0.05, 0.3, len(sales_data_df)
        )
        ...
        await insert_data(
            conn,
            "sales_data",
            sales_data_df[sales_data_cols],
            conflict_cols=["store_id", "product_id", "sale_date"],
        )
        ...
        weather_data_df["temp_min"] = weather_data_df[
            "avg_temperature"
        ] - np.random.uniform(2, 5, len(weather_data_df))
        weather_data_df["temp_max"] = weather_data_df[
            "avg_temperature"
        ] + np.random.uniform(2, 5, len(weather_data_df))
        ...
        weather_data_df["weather_condition"] = (
            "Clear"  # Dummy, can be derived from precpt/temp
        )
        ...
        await insert_data(
            conn,
            "weather_data",
            weather_data_df[weather_data_cols],
            conflict_cols=["city_id", "date"],
        )

        # Holiday Calendar
        holiday_calendar_df = (
            df_raw[df_raw["holiday_flag"] == 1][["dt", "holiday_flag"]]
            .drop_duplicates()
            .copy()
        )
        if not holiday_calendar_df.empty:
            holiday_calendar_df["date"] = pd.to_datetime(
                holiday_calendar_df["dt"]
            ).dt.date
            holiday_calendar_df["holiday_name"] = "Holiday " + holiday_calendar_df[
                "date"
            ].astype(
                str
            )  # Dummy name
            ...
            await insert_data(
                conn,
                "holiday_calendar",
                holiday_calendar_df[holiday_calendar_cols],
                conflict_cols=["date"],
            )
        ...
        if not promotion_events_df.empty:
            promotion_events_df["start_date"] = pd.to_datetime(
                promotion_events_df["dt"]
            ).dt.date
            promotion_events_df["end_date"] = promotion_events_df[
                "start_date"
            ] + pd.to_timedelta(
                7, unit="D"
            )  # Assume 7-day promo
            ...
            promotion_events_df["campaign_id"] = "CAMPAIGN_" + (
                promotion_events_df.index
            ).astype(str)
            ...
            promotion_events_df = promotion_events_df.drop_duplicates(
                subset=["store_id", "product_id", "start_date"]
            )
            await insert_data(
                conn,
                "promotion_events",
                promotion_events_df[promo_events_cols],
                conflict_cols=["store_id", "product_id", "start_date"],
            )
