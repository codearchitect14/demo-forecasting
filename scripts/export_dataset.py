...
def process_data(df):
    ...
    if "hours_sale" in processed_df.columns:
        processed_df["hours_sale_json"] = processed_df["hours_sale"].apply(
            lambda x: str(list(x))
        )
    ...
    if "hours_stock_status" in processed_df.columns:
        processed_df["hours_stock_status_json"] = processed_df[
            "hours_stock_status"
    ...
    if HAS_MAPPINGS and "city_id" in processed_df.columns:
        processed_df["city_name"] = processed_df["city_id"].map(
            lambda x: CITY_MAPPING.get(x, f"City {x}")
        )

    # Map store IDs to real names if mappings are available
    if HAS_MAPPINGS and "store_id" in processed_df.columns:
        processed_df["store_name"] = processed_df["store_id"].map(
            lambda x: STORE_MAPPING.get(x, f"Store {x}")
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Export FreshRetailNet-50K data to various formats"
    )
    parser.add_argument(
